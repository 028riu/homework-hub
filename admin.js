  ...
  if (loginBtn) {
    loginBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        console.error("Homework Hub Admin: Google sign-in failed", e);
        err(`Không thể đăng nhập Google: ${e.code || e.message}`);
      } finally {
...
function start(){
  ...
  const listen = (label, target, success, failure) => {
    const stop = onSnapshot(target, success, e => {
  ...
  listen("users", collection(db,"users"), s => {
  ...
  listen("subjects", query(collection(db,"subjects"),orderBy("order","asc")), s => {
    subjects=s.docs.map(d=>({id:d.id,...d.data()}));
    renderSubjects(); fillSubjectSelect(); renderCalendar();
  });

  listen("homework", query(collection(db,"homework"),orderBy("createdAt","desc")), s => {
    homeworks=s.docs.map(d=>({id:d.id,...d.data()}));
    renderHomework(); renderOverview(); renderStats(); renderCalendar();
  });

  listen("settings", collection(db,"settings"), s => {
    settings={}; s.docs.forEach(d=>settings[d.id]=d.data()); renderSettings();
  });

  listen("BME", query(collection(db,"bme_homework"),orderBy("createdAt","desc")), s => {
...
console.log("Homework Hub Admin: module loaded successfully");
