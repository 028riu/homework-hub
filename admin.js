...
function normalizeAttachments(homework) {
  ...
  const result = list.map((item) => ({
    ...
    url: item.url || item.fileUrl || item.linkUrl || "",
    name: item.name || item.fileName || (item.type === "file" ? "Tệp đính kèm" : "Liên kết"),
    fileName: item.fileName || item.name || "",
    fileType: item.fileType || "",
    size: normalizeNumber(item.size || item.fileSize, 0),
  ...
  if (!result.length && homework?.fileUrl) {
    result.push({
      ...
      fileName: homework.fileName || "Tệp đính kèm",
...
function attachmentRowsFromHomework(homework) {
  ...
  attachments.forEach((attachment) => addAttachmentRow(attachment.type, attachment));
}
  ...
  if (type === "link") {
    ...
    row.innerHTML = `
      ...
      <div class="attachment-main">
        ...
        <input class="attachment-existing-url" type="hidden" value="${esc(data?.url || "")}">
        <input class="attachment-existing-name" type="hidden" value="${esc(data?.name || data?.fileName || "")}">
        <input class="attachment-existing-path" type="hidden" value="${esc(data?.storagePath || "")}">
        <small>${esc(data?.name ? `File hiện tại: ${data.name}` : "Không bắt buộc. Có thể chọn nhiều file bằng cách bấm +.")}</small>
      </div>
      <button type="button" class="icon-btn remove-attachment" title="Xóa">×</button>
    `;
  }
...
function collectAttachmentRows() {
  ...
  return rows.map((row) => {
    ...
    return {
      type: "file",
      file: row.querySelector(".attachment-file")?.files?.[0] || null,
      existingUrl: row.querySelector(".attachment-existing-url")?.value || "",
      existingName: row.querySelector(".attachment-existing-name")?.value || "",
      existingPath: row.querySelector(".attachment-existing-path")?.value || ""
    };
  }).filter((item) => item.type === "link" ? item.url : (item.file || item.existingUrl));
}

async function uploadAttachment(file, homeworkId) {
  const safeName = safeFileName(file.name);
  const path = `homework/${homeworkId}/${Date.now()}_${crypto.randomUUID()}_${safeName}`;
...
async function deleteStorageFiles(attachments) {
  const files = normalizeAttachments({ attachments }).filter((item) => item.storagePath);
...
function renderAttachmentSummary(homework) {
  const attachments = normalizeAttachments(homework);
  if (!attachments.length) return "";
  return `<small>📎 ${attachments.length} tệp/link đính kèm</small>`;
}
...
$("addFileBtn")?.addEventListener("click", () => addAttachmentRow("file"));
...
$("homeworkForm")?.addEventListener("submit", async (event) => {
  ...
  for (const row of rows) {
    if (row.type === "file" && row.file && row.file.size > maxBytes) {
      errorEl.textContent = `File "${row.file.name}" vượt quá 50 MB.`;
      return;
    }
  }
  ...
  try {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      progressEl.textContent = `Đang xử lý tệp ${index + 1}/${rows.length}...`;

      if (row.type === "link") {
        attachments.push({ type: "link", url: row.url, name: row.url });
      } else if (row.file) {
        attachments.push(await uploadAttachment(row.file, homeworkId));
      } else if (row.existingUrl) {
        attachments.push({
          type: "file",
          url: row.existingUrl,
          name: row.existingName || "Tệp đính kèm",
          fileName: row.existingName || "Tệp đính kèm",
          storagePath: row.existingPath || ""
        });
      }
    }
    ...
    if (oldHomework) {
      const oldAttachments = normalizeAttachments(oldHomework);
      const newPaths = new Set(attachments.map((item) => item.storagePath).filter(Boolean));
      const removed = oldAttachments.filter((item) => item.storagePath && !newPaths.has(item.storagePath));
