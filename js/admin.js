...
document.addEventListener('click',async e=>{if(e.target.closest('#broadcastDirectNotice')){const msg=$("directNoticeText")?.value.trim();if(!msg)return alert('Hãy nhập nội dung thông báo.');try{const id=String(Date.now());await setDoc(doc(db,'settings','directNotification'),{id,title:'📢 Thông báo trực tiếp',message:msg,sentAt:serverTimestamp(),updatedAt:serverTimestamp(),active:true},{merge:true});showToast('Đã phát thông báo trực tiếp!')}catch(err){alert('Không thể phát: '+err.message)}}});
document.querySelectorAll('.admin-tab').forEach(b=>b.onclick=()=>{document.addEventListener('click',async e=>{if(e.target.closest('#broadcastDirectNotice')){const msg=$("directNoticeText")?.value.trim();if(!msg)return alert('Hãy nhập nội dung thông báo.');try{const id=String(Date.now());await setDoc(doc(db,'settings','directNotification'),{id,title:'📢 Thông báo trực tiếp',message:msg,sentAt:serverTimestamp(),updatedAt:serverTimestamp(),active:true},{merge:true});showToast('Đã phát thông báo trực tiếp!')}catch(err){alert('Không thể phát: '+err.message)}}});
document.querySelectorAll('.admin-tab').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.admin-page').forEach(p=>p.classList.toggle('hidden',p.dataset.page!==b.dataset.adminTab));if(b.dataset.adminTab==='calendar')renderCalendar()});
