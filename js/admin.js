...
function start() {
  ...
  unsubscribeSubjects = onSnapshot(
    ...
    (error) => {
      ...
      $("adminTabs").innerHTML = `<p class="error">Không thể tải môn học: ${esc(error.message)}</p>`;
    }
  );
  ...
  unsubscribeHomework = onSnapshot(
    ...
    (error) => {
      ...
      $("adminHomework").innerHTML = `<p class="error">Không thể tải bài tập: ${esc(error.message)}</p>`;
    }
  );
}
...
function renderHomework() {
  ...
  container.innerHTML = homeworks.map((homework) => {
    ...
    if (homework.dueDate) {
      ...
      if (!Number.isNaN(due.getTime())) dueText = due.toLocaleString("vi-VN");
    }

    return `
      <div class="admin-item">
        <b>${homework.pinned ? "📌 " : ""}${homework.important ? "⭐ " : ""}${esc(homework.title || "Bài tập")}</b>
        <small>${esc(subject?.icon || "📚")} ${esc(subject?.name || "Chưa phân loại")} · ${esc(dueText)}</small>
        <div class="actions">
...
function updateStats() {
  ...
  $("statTabs").textContent = String(subjects.length);
  $("statPinned").textContent = String(homeworks.filter((item) => item.pinned).length);
}
...
$("homeworkForm").addEventListener("submit", async (event) => {
  ...
  if (!subjectId) {
    $("hwError").textContent = "Vui lòng chọn môn học.";
    return;
  }
  if (!title) {
    $("hwError").textContent = "Vui lòng nhập tiêu đề.";
    return;
  }
  if (!content) {
    $("hwError").textContent = "Vui lòng nhập nội dung.";
    return;
  }
  ...
  if (id) {
    ...
    data.createdAt = oldHomework?.createdAt || serverTimestamp();
  } else {
  ...
  try {
    ...
    await setDoc(doc(db, "homework", documentId), data);
    $("hwError").textContent = "";
    $("homeworkDialog").close();
  } catch (error) {
    console.error("Save homework error:", error);
    $("hwError").textContent = "Không thể lưu: " + error.message;
  }
});
...
$("tabForm").addEventListener("submit", async (event) => {
  ...
  if (!name) {
    $("tabError").textContent = "Vui lòng nhập tên môn học.";
    return;
  }
  ...
  const order = id
    ? (oldSubject?.order || 0)
    : (subjects.length ? Math.max(...subjects.map((item) => item.order || 0)) + 1 : 1);

  const data = { name, icon, order, updatedAt: serverTimestamp() };

  try {
    const documentId = id || crypto.randomUUID();
    await setDoc(doc(db, "subjects", documentId), data);
    $("tabError").textContent = "";
    $("tabDialog").close();
  } catch (error) {
    console.error("Save subject error:", error);
    $("tabError").textContent = "Không thể lưu: " + error.message;
  }
});

// Event delegation for dynamic buttons
$("adminHomework").addEventListener("click", (event) => {
...
async function removeSubject(id) {
  const hasHomework = homeworks.some((item) => item.subjectId === id);
  if (hasHomework) {
    alert("Môn này đang có bài tập.\n\nHãy chuyển hoặc xóa các bài tập thuộc môn này trước.");
    return;
  }
