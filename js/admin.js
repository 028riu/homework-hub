...
function renderSettings(){const s=settings.site||{},direct=settings.directNotification||{};$('adminSettings').innerHTML=`
<label class="setting-card"><span><b>🔔 Thông báo không có bài mới</b><small>Bật cảnh báo khi hôm nay chưa có bài mới.</small></span><input id="setNo" type="checkbox" ${s.noHomeworkNoticeEnabled!==false?'checked':''}></label>
<label class="setting-card"><span><b>📢 Thông báo bài cũ</b><small>Bật cảnh báo khi danh sách chưa cập nhật.</small></span><input id="setOld" type="checkbox" ${s.oldHomeworkNoticeEnabled!==false?'checked':''}></label>
<label class="setting-card"><span><b>🎁 XP hoàn thành bài</b><small>Số XP mặc định hiển thị trên card.</small></span><input id="setXP" type="number" min="0" value="${num(s.xpPerHomework,30)}"></label>
<label class="setting-card"><span><b>💎 Points hoàn thành bài</b><small>Số Points mặc định.</small></span><input id="setPoints" type="number" min="0" value="${num(s.pointsPerHomework,20)}"></label>
<label>Tiêu đề thông báo<textarea id="setTitle" rows="2">${esc(s.noHomeworkNoticeTitle||'📚 Hôm nay không có bài tập mới')}</textarea></label>
<label>Nội dung thông báo<textarea id="setMsg" rows="3">${esc(s.noHomeworkNoticeMessage||'Hôm nay chưa có bài tập mới được cập nhật.')}</textarea></label>
<div class="setting-card direct-admin-notice"><span><b>📢 Thông báo trực tiếp</b><small>Phát ngay thông báo toàn màn hình tới View học sinh.</small></span><textarea id="directNoticeText" rows="4" placeholder="Nhập nội dung cần phát...">${esc(direct.message||'')}</textarea><button id="broadcastDirectNotice" type="button" class="primary small">📣 Phát thông báo</button></div>`}
$('saveAllSettings').onclick=async()=>{try{await setDoc(doc(db,'settings','site'),{noHomeworkNoticeEnabled:$('setNo').checked,oldHomeworkNoticeEnabled:$('setOld').checked,xpPerHomework:num($('setXP').value,30),pointsPerHomework:num($('setPoints').value,20),noHomeworkNoticeTitle:$('setTitle').value.trim(),noHomeworkNoticeMessage:$('setMsg').value.trim(),updatedAt:serverTimestamp()},{merge:true});showToast('Đã lưu cài đặt')}catch(e){alert(e.message)}};
document.addEventListener('click',async e=>{if(!e.target.closest('#broadcastDirectNotice'))return;const msg=$("directNoticeText")?.value.trim();if(!msg)return alert('Hãy nhập nội dung thông báo.');try{const id=String(Date.now());await setDoc(doc(db,'settings','directNotification'),{id,title:'📢 Thông báo trực tiếp',message:msg,sentAt:serverTimestamp(),updatedAt:serverTimestamp(),active:true},{merge:true});showToast('Đã phát thông báo trực tiếp!')}catch(err){alert('Không thể phát: '+err.message)}});
